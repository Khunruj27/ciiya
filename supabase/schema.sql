...
-- =========================================================
-- SHARE EVENTS: owner notifications and gallery analytics
-- =========================================================
...
create table if not exists public.share_events (
  ...
  constraint share_events_type_check check (
    event_type in ('album_view', 'photo_download', 'photo_like', 'moment_created', 'moment_like', 'face_search')
  )
);
...
create table if not exists public.announcements (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint announcements_type_check check (announcement_type in ('feature', 'promotion', 'update', 'maintenance', 'tip', 'security')),
  constraint announcements_audience_check check (audience_type in ('all', 'plans', 'users')),
  constraint announcements_priority_check check (priority in ('normal', 'important', 'critical')),
  constraint announcements_status_check check (status in ('draft', 'scheduled', 'published', 'expired', 'cancelled')),
...
create policy "announcements_recipient_select" on public.announcements
for select to authenticated using (
  status in ('published', 'scheduled') and starts_at <= now()
  and (expires_at is null or expires_at > now())
  and (
    audience_type = 'all'
    or (audience_type = 'users' and auth.uid()::text = any(audience_values))
    or (
      audience_type = 'plans' and (
        exists (
        ...
        or (
          ...
          and not exists (select 1 from public.user_storage_usage usage where usage.user_id = auth.uid())
        )
      )
    )
  )
...
create policy "announcement_reads_owner_select" on public.announcement_reads
for select to authenticated using (user_id = auth.uid());
create policy "announcement_reads_owner_insert" on public.announcement_reads
...
create policy "announcement_reads_owner_update" on public.announcement_reads
...
create table if not exists public.ai_jobs (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ai_jobs_feature_check check (feature in (
    'gallery_assistant', 'portfolio_designer', 'client_selection',
    'analytics_summary', 'notification_writer', 'operations_assistant'
  )),
  constraint ai_jobs_status_check check (status in ('pending', 'processing', 'completed', 'failed', 'cancelled')),
...
create index if not exists idx_ai_jobs_owner_created on public.ai_jobs(owner_id, created_at desc);
create index if not exists idx_ai_jobs_album_feature_created on public.ai_jobs(album_id, feature, created_at desc) where album_id is not null;
create index if not exists idx_ai_jobs_pending on public.ai_jobs(status, created_at) where status in ('pending', 'processing');
...
create table if not exists public.ai_suggestions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ai_suggestions_status_check check (status in ('pending', 'applied', 'dismissed')),
  constraint ai_suggestions_confidence_check check (confidence is null or (confidence >= 0 and confidence <= 1))
);

create index if not exists idx_ai_suggestions_owner_status on public.ai_suggestions(owner_id, status, created_at desc);
create index if not exists idx_ai_suggestions_album_feature on public.ai_suggestions(album_id, feature, created_at desc) where album_id is not null;
...
create table if not exists public.ai_audit_logs (
  ...
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  constraint ai_audit_logs_actor_check check (actor_type in ('user', 'system', 'worker', 'admin'))
);

create index if not exists idx_ai_audit_logs_owner_created on public.ai_audit_logs(owner_id, created_at desc);
...
create policy "ai_jobs_owner_select" on public.ai_jobs for select to authenticated using (owner_id = auth.uid());
create policy "ai_suggestions_owner_select" on public.ai_suggestions for select to authenticated using (owner_id = auth.uid());
create policy "ai_suggestions_owner_update" on public.ai_suggestions for update to authenticated
using (owner_id = auth.uid()) with check (owner_id = auth.uid());
create policy "ai_audit_logs_owner_select" on public.ai_audit_logs for select to authenticated using (owner_id = auth.uid());
