...
function formatMomentTime(value: string, t: Dictionary, locale: Locale) {
  const date = new Date(value)
  const difference = Date.now() - date.getTime()
  const minutes = Math.max(1, Math.floor(difference / 60_000))

  if (minutes < 60) return t.moments.minAgo(minutes)
  const hours = Math.floor(minutes / 60)
  if (hours < 24) return t.moments.hrAgo(hours)
...
export default function GuestMoments({ token, active, onCountChange }: Props) {
  ...
  function chooseFiles(nextFiles: File[]) {
    ...
    setFiles(images)
    setFormError(nextFiles.length > MAX_FILES ? t.moments.maxPhotos(MAX_FILES) : '')
  }
  ...
  function submitMoment(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!guestName.trim()) {
      setFormError(t.moments.enterName)
      return
    }

    if (files.length === 0) {
      setFormError(t.moments.choosePhoto)
      return
    }
    ...
    request.onerror = () => {
      setUploading(false)
      setFormError(t.moments.uploadInterrupted)
    }
    request.onload = () => {
      const response = request.response
      if (request.status < 200 || request.status >= 300 || !response?.success) {
        setUploading(false)
        setFormError(response?.error || t.moments.unableShare)
        return
      }
  ...
  async function shareMoment(moment: GuestMoment) {
    ...
    try {
      ...
      setShareStatus(t.moments.shared)
  ...
  async function toggleMomentLike(moment: GuestMoment) {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok || !data.success) throw new Error(data?.error || t.moments.reactionFailed)
  ...
  function shareToFacebook(moment: GuestMoment) {
    ...
    setShareStatus(t.moments.fbOpened)
  ...
  async function shareToInstagram(moment: GuestMoment, imageUrl: string) {
    setStoryPaused(true)
    setShareStatus(t.moments.preparing)
    ...
    try {
      ...
      setShareStatus(t.moments.readyToShare)
    } catch {
      setShareStatus(t.moments.shareCancelled)
    } finally {
      window.setTimeout(() => setShareStatus(''), 1800)
      setStoryPaused(false)
    }
  ...
  return (
    <div className="space-y-5">
      <section className="overflow-hidden rounded-hero border border-line bg-surface shadow-card">
        <div className="relative px-5 py-7 sm:px-8 sm:py-9">
          ...
          <div className="relative flex flex-col items-center gap-7">
            ...
            >
              <PlusIcon /> {t.moments.shareAMoment}
            </button>
            ...
            {loading ? (
              ...
              <div className="w-full rounded-panel border border-line bg-ground-sunken px-6 py-10 text-center">
                <p className="text-[15px] font-semibold">{t.moments.unavailable}</p>
              ...
              <div className="flex w-full flex-col items-center py-6 text-center">
                ...
                <span className="mt-5 text-[19px] font-semibold">{t.moments.emptyTitle}</span>
                <span className="mt-2 max-w-sm text-[13px] leading-6 text-muted">{t.moments.emptySub}</span>
              </div>
            ) : (
              <div className="grid w-full max-w-4xl grid-cols-4 gap-x-0 gap-y-4 sm:gap-x-2 sm:gap-y-7 lg:gap-x-1">
                {moments.map((moment, momentIndex) => (
                  <button
                    ...
                    aria-label={t.moments.openMomentFrom(moment.guest_name)}
                  >
                    <span className="moment-story-ring relative block aspect-square w-full max-w-[96px] overflow-hidden rounded-full bg-gold p-[2px] shadow-card transition duration-300 group-hover:-translate-y-0.5 group-hover:shadow-lift group-active:scale-95 sm:max-w-[180px] lg:max-w-[220px]">
                      <span className="relative z-10 block h-full w-full rounded-full bg-surface p-[3px]">
                        <img src={moment.image_urls[0]} alt={t.moments.momentBy(moment.guest_name)} loading="lazy" className="h-full w-full rounded-full object-cover" />
                      </span>
                    ...
                    <span className="mt-1 block text-center text-[9px] font-normal leading-none text-muted sm:text-[11px]">{formatMomentTime(moment.created_at, t, locale)}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      {composerOpen ? (
        <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/55 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label={t.moments.shareGuestMoment}>
          <form onSubmit={submitMoment} className="max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-[26px] bg-surface p-5 shadow-float sm:rounded-hero sm:p-7">
            ...
            <input value={guestName} onChange={(event) => setGuestName(event.target.value.slice(0, 60))} disabled={uploading} placeholder={t.moments.namePlaceholder} className="mt-2 h-12 w-full rounded-control border border-line bg-ground px-4 text-[14px] outline-none transition focus:border-gold disabled:opacity-60" />

            <label className="mt-5 block text-[12px] font-semibold">{t.moments.photos}</label>
            <label className="mt-2 flex min-h-32 cursor-pointer flex-col items-center justify-center rounded-panel border border-dashed border-line-strong bg-ground px-4 py-5 text-center transition hover:border-gold">
              ...
              <span className="mt-3 text-[13px] font-semibold">{t.moments.chooseUpTo}</span>
              <span className="mt-1 text-[10px] text-muted">{t.moments.fileHint}</span>
            </label>

            {previews.length > 0 ? (
              <div className="mt-3 grid grid-cols-4 gap-2">
                {previews.map((url, index) => <img key={url} src={url} alt={t.moments.selectedPhoto(index + 1)} className="aspect-square w-full rounded-card object-cover" />)}
              </div>
            ) : null}

            <label className="mt-5 block text-[12px] font-semibold">{t.moments.message} <span className="font-normal text-muted">{t.moments.optional}</span></label>
            <textarea value={message} onChange={(event) => setMessage(event.target.value.slice(0, 280))} disabled={uploading} rows={3} placeholder={t.moments.messagePlaceholder} className="mt-2 w-full resize-none rounded-control border border-line bg-ground px-4 py-3 text-[14px] leading-6 outline-none transition focus:border-gold disabled:opacity-60" />
            ...
            {uploading ? (
              <div className="mt-4"><div className="flex items-center justify-between text-[11px] font-medium"><span>{uploadProgress < 100 ? t.moments.sharing : t.moments.momentShared}</span><span>{uploadProgress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-ground-sunken"><div className="h-full rounded-full bg-gold transition-[width] duration-300" style={{ width: `${uploadProgress}%` }} /></div></div>
            ) : null}

            <button type="submit" disabled={uploading} className="mt-5 flex h-12 w-full items-center justify-center rounded-full bg-ink px-6 text-[13px] font-semibold text-white transition active:scale-[0.99] disabled:opacity-60">{uploading ? t.moments.uploading : t.moments.submit}</button>
          </form>
        </div>
      ) : null}

      {activeStory && activeStoryMoment && activeStoryImage ? (
        <div className="fixed inset-0 z-[130] grid place-items-center bg-black sm:bg-black/95 sm:p-5" role="dialog" aria-modal="true" aria-label={t.moments.storyDialog}>
          <div className="relative h-[100dvh] w-full max-w-[460px] select-none overflow-hidden bg-[#111] text-white shadow-2xl sm:h-[min(900px,94dvh)] sm:rounded-[28px]" onContextMenu={(event) => event.preventDefault()}>
            ...
            <img src={activeStoryImage} alt={t.moments.momentBy(activeStoryMoment.guest_name)} className="pointer-events-none absolute inset-0 h-full w-full object-contain" />

            <button type="button" onClick={() => handleStoryTap('previous')} onPointerDown={startStoryPress} onPointerUp={endStoryPress} onPointerCancel={cancelStoryPress} className="absolute inset-y-0 left-0 z-10 w-[38%] touch-none cursor-w-resize" aria-label={t.moments.prevStory} />
            <button type="button" onClick={() => handleStoryTap('next')} onPointerDown={startStoryPress} onPointerUp={endStoryPress} onPointerCancel={cancelStoryPress} className="absolute inset-y-0 right-0 z-10 w-[62%] touch-none cursor-e-resize" aria-label={t.moments.nextStory} />
            ...
            <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 bg-gradient-to-t from-black/90 via-black/45 to-transparent px-4 pb-[max(18px,env(safe-area-inset-bottom))] pt-28 sm:px-5 sm:pb-6">
              ...
              <div className="pointer-events-auto mt-4 flex items-center gap-2">
                <button
                  ...
                  aria-label={activeStoryLiked ? t.moments.unlike : t.moments.like}
                  aria-pressed={activeStoryLiked}
                >
                  <HeartIcon filled={activeStoryLiked} />
                  <span>{Number(activeStoryMoment.like_count || 0)}</span>
                </button>
                <button type="button" onClick={() => shareToFacebook(activeStoryMoment)} className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/25 bg-black/25 text-white backdrop-blur transition active:scale-95" aria-label={t.moments.shareToFb}><FacebookIcon /></button>
                <button type="button" onClick={() => shareToInstagram(activeStoryMoment, activeStoryImage)} className="grid h-11 w-11 shrink-0 place-items-center rounded-full border border-white/25 bg-black/25 text-white backdrop-blur transition active:scale-95" aria-label={t.moments.shareToIg}><InstagramIcon /></button>
                <button type="button" onClick={() => shareMoment(activeStoryMoment)} className="flex h-11 min-w-0 flex-1 items-center justify-center gap-2 rounded-full border border-white/25 bg-white px-3 text-[11px] font-semibold text-ink transition active:scale-[0.98]" aria-label={t.moments.moreShare}><ShareIcon /> <span className="truncate">{sharedId === activeStoryMoment.id ? t.moments.shared : t.moments.share}</span></button>
              </div>
