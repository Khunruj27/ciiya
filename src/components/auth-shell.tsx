  ...
  return (
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <Link href={signup ? '/login' : '/signup'} className={styles.switchLink}>{signup ? t.signup.signIn : t.login.createAccount}<ArrowUpRight size={15} aria-hidden /></Link>
      </header>
      <div className={styles.layout}>
        <aside className={styles.story}>
          <Image src={`/landing/${signup ? 'editorial-couple' : 'editorial-walk'}.webp`} alt={thai ? 'ภาพคู่บ่าวสาวในสวน' : 'A wedding portrait in a garden'} fill sizes="(max-width: 900px) 1px, 50vw" className={styles.photo} />
        ...
        <section className={styles.formPanel}>
          <div className={styles.formContent}>
            <p className={styles.formEyebrow}>{signup ? t.signup.badge : t.login.welcome}</p>
            <h1>{signup ? (thai ? 'เริ่มเก็บช่วงเวลาดี ๆ' : 'Make room for moments.') : (thai ? 'ยินดีที่ได้พบกันอีกครั้ง' : 'Good to see you again.')}</h1>
            <p className={styles.introduction}>{signup ? t.signup.subtitle : t.login.subtitle}</p>
            {children}
          </div>
