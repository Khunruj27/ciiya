  ...
  return (
    <main className="min-h-dvh overflow-x-hidden bg-[#F9F9F9] text-black">
      <div className="mx-auto flex min-h-dvh w-full max-w-[393px] flex-col px-4 pt-[max(54px,env(safe-area-inset-top))] pb-[calc(104px+env(safe-area-inset-bottom))]">
        ...
        <section className="pt-6">
          <div className="h-[132px] w-full animate-pulse rounded-[28px] bg-black/10" />
        </section>
        ...
        <section className="pt-5">
          ...
          <div className="space-y-3">
            {Array.from({ length: 3 }).map((_, i) => (
              <div
                key={i}
                className="flex w-full gap-3 rounded-[22px] bg-white p-2 border border-black/5"
              >
                <div className="h-[86px] w-[96px] shrink-0 animate-pulse rounded-[18px] bg-black/10" />
